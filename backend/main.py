...
if (_MEDIA_DIR / "media" / "photos").is_dir():
    app.mount("/media/photos", StaticFiles(directory=str(_MEDIA_DIR / "media" / "photos")), name="media_photos")
if (_MEDIA_DIR / "media" / "voice").is_dir():
    app.mount("/media/voice", StaticFiles(directory=str(_MEDIA_DIR / "media" / "voice")), name="media_voice")
...
def ai_provider_snapshot() -> dict:
    ...
    if provider == "dashscope":
        ...
        models = {
            "stt": slice_runner._normalize_gemini_model(os.environ.get("GEMINI_FLASH_MODEL", os.environ.get("GEMINI_STT_MODEL", "gemini-3.1-flash-lite"))),
            "vision": slice_runner._normalize_gemini_model(os.environ.get("GEMINI_FLASH_MODEL", os.environ.get("GEMINI_VISION_MODEL", "gemini-3.1-flash-lite"))),
            "classifier": slice_runner._normalize_gemini_model(os.environ.get("GEMINI_CLASSIFIER_MODEL", os.environ.get("GEMINI_FLASH_MODEL", "gemini-3.1-flash-lite"))),
        }
...
def _ensure_port_available(port: int) -> None:
    """If port is occupied by a stale/zombie process, release it so uvicorn binds cleanly."""
    import socket
    ...
    _warn(f"Port {port} is already in use. Checking for stale process...")
    import subprocess
    import time
    my_pid = os.getpid()
    try:
            ...
            for line in out.splitlines():
                if f":{port}" in line and "LISTENING" in line:
                    ...
                    if pid != my_pid and pid > 0:
                        _log(f"Terminating stale process holding port {port} (PID {pid})...")
        ...
        _warn(f"Could not automatically free port {port}: {exc}")
